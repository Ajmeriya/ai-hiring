...
def get_testcases(question_id):
    ...
    with urllib.request.urlopen(url) as resp:
        ...
        try:
            ...
            print('Failed to parse testcases:', e)
            print(data)
            sys.exit(1)


def post_submission(payload):
    url = 'http://localhost:8090/api/execute/submit'
...
if __name__ == '__main__':
    ...
    # tcs might be a dict with 'value'
...
'''
    ...
    print('Posting payload with', len(cases), 'test cases (hidden count =', sum(1 for c in cases if c.get('hidden')), ')')
    resp = post_submission(payload)
